...
- `frontend/src/components/features/dossier/` - Dossier is split across multiple files (was a 1366-line monolith): `DossierPage.tsx` is a thin shell that routes list/detail via `?dossier=<id>`, `DossierListView.tsx` hosts the grid + card context menu, `DossierDetailView.tsx` hosts the single-dossier view. Modals live one-per-file (`EditDossierModal`, `ImportDossierModal`, `MoveToDossierModal`, `TreeNavigatorModal`, `ArticleViewerModal`, `OpenOnDashboardPicker`). Shared helpers (`formatTimestampLong`, `STATUS_CONFIG`, `computeNormaGroups`, `NormaGroup`) live in `dossierUtils.ts`. The item row component is `SortableDossierItem.tsx`. Never add new dossier features inline in `DossierPage.tsx` — it's intentionally minimal.
